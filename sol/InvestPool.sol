	...
	*/
	function getTokens() public returns(bool) {
		...
		if (msg.sender == poolOwner) {
			value = tokenBalance.mul(poolCommision).div(100);
		} else {
			value = tokenBalance.sub(tokenBalance.mul(poolCommision).div(100))
				.mul(investments[msg.sender].mul(10000).div(balance)).div(10000);
		}
